...
int main(int argc, char *argv[])
{
  ...
  actualDistancesFile.open(folder + graph_name + "_PLL_distances.txt");
  actualRuntimesFile.open(folder + graph_name + "_PLL_runtimes.txt");
  actualSampleFile.open(folder + graph_name + "_PLL_failed.txt");
  ...
  for(int round = 0; round < ROUNDS; round ++){
      ...
      VertexIdx v2;
      inputFile >> v1 >> v2;
      if(round < startFrom) continue;

      if (round%1000==0) {
          printf("round %i\n", round);
          for (const auto &e : actualDistances) actualDistancesFile << e << "\n";
          for (const auto &e : actualRuntimes) actualRuntimesFile << e << "\n";
