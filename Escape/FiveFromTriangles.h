...
namespace Escape
{
  ...
  Count count5_Hourglass(const CGraph *g, const TriangleInfo *tInfo = 0, Count *counts = 0)
  {
    if (!tInfo)
    {
  ...
  Count count5_Stingray(const CGraph *gOut, const CGraph *gIn, const TriangleInfo *tInfo = 0, Count *counts = 0)
  {
    if (!tInfo)
    {
      // compue here todo
  ...
  //   Count
  template <bool doPerEdge>
  Count count5_StellateTrident(const CGraph *g, const TriangleInfo *tInfo = 0, Count *counts = 0)
  {
    if (!tInfo)
    {
      // todo
  ...
  Count count5_TriangleStrip(const CGraph *gOut, Count k4, const TriangleInfo *tInfo_ = 0)
  {
    ...
    if (!isDOG)
    {
      ...
      tInfo.total = countTriangles(&dog.outlist, true, true, &dog.inlist, tInfo.perVertex, tInfo.perEdge);
      g = &dog.outlist;
    }
    else
    {
