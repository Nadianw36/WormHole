...
namespace Escape
{
  ...
  void triangleProgram(const CGraph *gOutList, F &functor, const CGraph *gInList = 0)
  {
    ...
    for (VertexIdx v = 0; v < gOutList->nVertices; ++v)
    {
      if (doDegreeOrdering)
      {
        ...
        {
          return totalDegrees[v1] < totalDegrees[v2] || (totalDegrees[v1] == totalDegrees[v2] && v1 < v2);
        };
  ...
  //   TriangleProgram
  int64_t countTriangles(const CGraph *gOut, bool degreeOrdered = false, bool directed = true, const CGraph *gIn = 0, int64_t *perVertex = 0, int64_t *perEdge = 0);
