...
# ./mll query -d {graphname}_seed_{size_str}_core-COO_sanitized -q ../inputs/MLL-inputs/{graphname}/{graphname}_seed_{size_str}_highest-deg-L0.txt
./mll query -d {graphname}_seed_{size_str}_core-COO_sanitized -q ../inputs/MLL-inputs/{graphname}/{graphname}_seed_{size_str}_all-pairs-L0.txt
"""
...
exe/generateL0COO {graphname}_seed_{size_str}
python python/sanitize.py {graphname}_seed_{size_str}_core-COO
python python/remap_inputs.py {graphname} {size_str} 0 10000
"""
...
def create_submit_script(graph_dict, name):
    ...
    for graphname, params in graph_dict.items():
        if params["mll-on-core"]:
            for size in params["l0_sizes"]:
                size_str = size_to_str(size)
                sbatch_run_scripts_mll_on_core += f"\n./MLL-on-core_{graphname}_{size_str}.sh"
